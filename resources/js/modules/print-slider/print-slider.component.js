define([
	...
	var elementProto = function() {
		...
		var Animation = function(line, value, speed, delay, pool) {
			this.line = line;
			this.value = '';
			this.speed = speed;
			this.delay = delay  || 0;
		...
		var createdCallback = function() {
			this._haldleSlideStore = haldleSlideStore.bind(this);
		}
		var attachedCallback = function() {
			...
			Array.prototype.forEach.call(lines, function(line, index) {
				...
				self._lines.push({
					element: line,
					animation: new Animation(lineObj, null, animationSpeed, index*150, self._pool)
				});
			});

			Array.prototype.forEach.call(sections, function(section, index) {
				var textElement = section.querySelector('.data-hidden[data-id="' + self._id + '"]');
				var lines;
			...
			console.log(self._textData);

			this._haldleSlideStore();
			slideStore.eventEmitter.subscribe(this._haldleSlideStore);
		}
		var detachedCallback = function() {
			slideStore.eventEmitter.unsubscribe(this._haldleSlideStore);
		}
